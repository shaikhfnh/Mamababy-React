 ...
 * @param {Object} acf - The full ACF data object
 * @param {string} language - Current language ('en' or 'ar')
 */
export default function ImpactStats({ acf }) {
    const { language} = useLanguage()
  if (!acf) return null;

  // Dynamically get all impact images based on language
  const impactImages = Object.keys(acf)
    .filter(key => {
  ...
  return (
    <section  style={{
        backgroundImage: `url(${acf?.background_image_benifit})`,
...
>
           {getLocalizedField(acf, `impact_heading`, language)}
          </div>
          <div className="mt-3 text-gray-800 max-w-xl mx-auto">
                     {getLocalizedField(acf, `impact_subheading`, language)}
